...
export class UserMenuController {
  ...
  private readonly devices: TrustedDevicesController;
  ...
  constructor(private readonly options: UserMenuControllerOptions) {
    ...
    this.devices = new TrustedDevicesController(options.menu.querySelector(".trusted-device-section")!, () => options.getUser()?.id);
    this.bindEvents();
  }
  ...
  close() {
    ...
    const adminMenu = this.options.menu.querySelector<HTMLDetailsElement>('#admin-actions');
    if (adminMenu) adminMenu.open = false;
  }

  private bindEvents() {
    // The canvas closes topbar menus on document clicks; internal disclosures stay open.
    this.options.menu.addEventListener('click', event => event.stopPropagation());
    ...
    this.options.button.addEventListener("click", (event) => {
      ...
      if (opening) {
        ...
        void Promise.all([this.loadTokenState(), this.devices.load()]);
      }
    });
