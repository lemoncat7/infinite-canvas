...
function syncDomNodes() {
  ...
  for (const node of nodes) {
    ...
    if (node.kind === 'video') {
      ...
      element.querySelector<HTMLElement>('.video-generation-count')!.textContent = node.role === 'result' ? node.status === 'queued' ? '任务排队中' : node.status === 'running' ? Number(node.progress ?? 0) > 0 ? `生成中 ${Math.round(node.progress ?? 0)}%` : node.model?.startsWith('agnes-') ? '云端处理中' : '生成中 · 等待进度' : node.status === 'failed' ? '生成失败' : '生成结果' : `已生成 ${count} 个视频`
...
function deleteSelectedNode() {
  ...
  if (index < 0) return
  if (nodeIsGenerationProtected(nodes[index])) { showToast('该节点正被生成任务使用，完成后即可删除', 'warning'); return }
...
function canGenerateNode(node: FlowNode) { const grokLocked = node.model === 'grok-imagine-video-1.5-preview' && Number(authUser?.credits ?? 0)-Number(authUser?.reservedCredits ?? 0)<2; return (node.kind === 'image' || node.kind === 'video') && node.role !== 'result' && !grokLocked && Boolean(node.body.trim()) }
