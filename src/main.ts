...
function createDomNode(node: FlowNode) {
  ...
  const commitDimensions = () => { const widthValue = Number(imagePanel.querySelector<HTMLInputElement>('[data-image-width]')!.value), heightValue = Number(imagePanel.querySelector<HTMLInputElement>('[data-image-height]')!.value); if (!widthValue || !heightValue) return; const width = Math.max(1, widthValue), height = Math.max(1, heightValue), select = imagePanel.querySelector<HTMLSelectElement>('[data-image-field="size"]')!; const value = `${Math.ceil(width / 16) * 16}x${Math.ceil(height / 16) * 16}`; if (![...select.options].some(option => option.value === value)) select.add(new Option(value, value)); select.value = value; select.dispatchEvent(new Event('change')); draw() }
  imagePanel.querySelectorAll<HTMLInputElement>('[data-image-width],[data-image-height]').forEach(input => { input.addEventListener('change', commitDimensions); input.addEventListener('keydown', event => { if (event.key === 'Enter') input.blur() }) })
