...
type PromptAgentStep = { title?:string; kind:'image'|'video'; prompt:string; referenceIndexes?:number[] }
...
function closePromptAgent(){promptAgentPanel.classList.remove('open','forming');promptAgentTrigger.classList.remove('active');promptAgentSelecting=false;if(promptAgentEffectFrame!==null)cancelAnimationFrame(promptAgentEffectFrame);promptAgentEffectFrame=null;promptAgentEffects.hidden=true;promptAgentEffectsFront.hidden=true;promptAgentRibbonBack.classList.remove('visible');promptAgentRibbonFront.classList.remove('visible');draw()}
function dispersePromptAgent(){if(!promptAgentPanel.classList.contains('open'))return;promptAgentPanel.classList.add('dispersing');window.setTimeout(()=>{promptAgentPanel.classList.remove('dispersing');closePromptAgent()},360)}
...
promptAgentTrigger.addEventListener('click',event=>{event.stopPropagation();if(!promptAgentPanel.classList.contains('open'))formPromptAgent()})
...
function applyPromptAgentPlan(result:PromptAgentResult){
  const sources=promptAgentContextNodes.filter(node=>promptAgentContextSelection.has(node.id)),current=sources[0],kind=result.targetType||result.kind,canUpdate=current&&current.kind===kind&&current.role!=='result'&&!current.mediaUrl,action=result.action==='update_current'&&canUpdate?'update_current':result.action==='create_new'?'create_new':'create_child'
  promptAgentUndo=null;promptAgentAppliedNodeId=0
  const planned=(result.steps||[]).filter(step=>(step.kind==='image'||step.kind==='video')&&step.prompt?.trim()).slice(0,8)
  if(planned.length){
    const imageSources=sources.filter(source=>source.kind==='image'&&Boolean(source.mediaUrl)),createdIds:number[]=[],base=current?{x:current.x+current.width+120,y:current.y}:{...world({x:innerWidth/2,y:innerHeight/2})}
    planned.forEach((step,index)=>{addNode(step.kind,{x:base.x,y:base.y+index*270});const created=nodes.find(node=>node.id===selectedId);if(!created)return;created.body=step.prompt.trim();created.generationPrompt=step.prompt.trim();created.title=step.title?.trim()||`Agent · ${step.kind==='video'?'视频':'图像'} ${index+1}`;createdIds.push(created.id);const indexes=(step.referenceIndexes||[]).map(Number).filter(value=>Number.isInteger(value)&&value>=1&&value<=imageSources.length),references=indexes.map(value=>imageSources[value-1]);references.forEach(source=>{if(!links.some(link=>link.from===source.id&&link.to===created.id))links.push({from:source.id,to:created.id,fromSide:'right',toSide:'left'})})})
    promptAgentAppliedNodeId=createdIds[0]||0;promptAgentUndo=()=>{for(let index=links.length-1;index>=0;index--)if(createdIds.includes(links[index].from)||createdIds.includes(links[index].to))links.splice(index,1);for(let index=nodes.length-1;index>=0;index--)if(createdIds.includes(nodes[index].id))nodes.splice(index,1);selectedId=0;promptAgentAppliedNodeId=0;scheduleSave();draw()};selectedId=promptAgentAppliedNodeId;scheduleSave();draw();return
  }
  if(action==='update_current'&&current){
    ...
    if(action==='create_child')sources.filter(source=>source.id!==created.id).forEach(source=>{if(!links.some(link=>link.from===source.id&&link.to===created.id))links.push({from:source.id,to:created.id,fromSide:'right',toSide:'left'})})
...
promptAgentPanel.querySelector<HTMLButtonElement>('.agent-submit')!.addEventListener('click',async()=>{const textarea=promptAgentPanel.querySelector<HTMLTextAreaElement>('textarea')!,idea=textarea.value.trim(),submit=promptAgentPanel.querySelector<HTMLButtonElement>('.agent-submit')!,status=promptAgentPanel.querySelector<HTMLOutputElement>('.agent-status')!,article=promptAgentPanel.querySelector<HTMLElement>('article')!,selected=nodes.find(node=>node.id===selectedId);if(!idea){showToast('先告诉我你想创造什么','warning');return}promptAgentKind=/视频|动态|动起来|镜头|分镜/.test(idea)?'video':'image';const selectedContexts=promptAgentContextNodes.filter(node=>promptAgentContextSelection.has(node.id)),context=selectedContexts.map((node,index)=>`${index===0?'当前节点':`参考节点${index+1}`}「${node.title}」：${node.generationPrompt||node.body||'无文字说明'}`),visuals=selectedContexts.filter(node=>node.kind==='image'&&node.mediaUrl).map(node=>node.mediaUrl!);submit.disabled=true;submit.classList.add('is-running');status.textContent='正在理解素材并规划画布…';article.hidden=true;try{const response=await fetch('/api/agents/prompt',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({idea,kind:promptAgentKind,complexity:promptAgentComplexity,context,visuals,model:promptAgentModelSelect.value,target:selected?{id:selected.id,kind:selected.kind,role:selected.role||'generator',hasMedia:Boolean(selected.mediaUrl),hasPrompt:Boolean((selected.generationPrompt||selected.body).trim())}:null})}),result=await response.json() as PromptAgentResult&{error?:string};if(!response.ok)throw new Error(result.error||'创作规划失败');promptAgentResult=result;applyPromptAgentPlan(result);playAgentMeteor();article.querySelector<HTMLElement>('[data-agent-prompt]')!.textContent=result.finalPrompt;article.querySelector<HTMLElement>('[data-agent-summary]')!.textContent=result.summary||'已根据你的素材准备好画布节点';article.querySelector('small')!.textContent=`${result.model} · ${(result.targetType||result.kind)==='video'?'视频':'图像'} · ${selectedContexts.length} 个参考`;article.querySelector<HTMLButtonElement>('[data-agent-undo]')!.hidden=!promptAgentUndo;status.textContent='画布已更新';textarea.value='';showToast(result.summary||'创作节点已准备','success')}catch(error){const message=error instanceof Error?error.message:'创作规划失败';status.textContent=message;showToast(message,'error')}finally{submit.disabled=false;submit.classList.remove('is-running')}})
