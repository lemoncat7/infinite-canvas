...
管理员功能统一位于账号菜单的「管理员」二级菜单，包含全局模型和充值码管理。登录设备默认显示 3 台，每次可再展开 5 台，当前设备优先显示；可单独退出设备或退出其他设备。

同一账号在同一浏览器重新登录会替换旧设备登记，同时使旧会话失效。浏览器使用独立的随机标识识别（不是登录凭据，退出后保留），不使用 IP 或浏览器名称推断设备；清除 Cookie、无痕窗口或不同浏览器会视为新设备。历史上无法可靠识别的重复登记不会自动合并，可通过「退出其他设备」清理。

## 技术栈

画布仅在生成中的连线上显示低帧率流动，使用缓存的连线几何，不在动画帧中遍历节点或重建路径；完成或排队状态保持静止。WebGL 分辨率最高为 1.5 倍；DOM 卡片、文字不降分辨率。页面隐藏、窗口失焦或画布不可见时暂停提交画布渲染，恢复后绘制最新状态；任务本身不会因此暂停。系统开启减少动态效果时不启用连线流动。
