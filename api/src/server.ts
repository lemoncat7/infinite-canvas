...
app.post('/agents/comic', async (request, reply) => {
  const user = requireUser(request, reply); if (!user) return
  const input = request.body as { idea?:string; duration?:string; aspectRatio?:string; context?:string[]; visuals?:string[]; previousPlan?:unknown; revision?:string; model?:string }
  ...
  const system=`你是 Viora 的漫剧导演与编剧。把用户创意规划成可执行的短篇漫剧，但此阶段只做策划，不声称已经生成图片或视频。用户没有说明时长或画幅时，根据内容规模主动选择合适的总时长和 9:16、16:9 或 1:1，不要反问，也不要使用“由对话内容推断”作为最终值。只返回合法 JSON，不要 Markdown。结构必须为：{"title":"片名","logline":"一句话梗概","tone":"明确的美术风格、时代、环境与情绪基调","duration":"总时长","aspectRatio":"画幅","characters":[{"name":"角色名","description":"外貌、服装、性格、动机与关键关系的稳定设定"}],"outline":[{"act":"剧情段落","content":"包含因果、冲突、转折和人物状态变化的详细剧情"}],"shots":[{"number":1,"title":"镜头标题","duration":5,"scene":"景别、人物位置、动作、环境与画面结果","dialogue":"准确对白或旁白，没有则为空","imagePrompt":"可独立生成静帧的完整提示词","videoPrompt":"承接静帧的动作、运镜、环境变化与镜头结束状态","transition":"与下一镜头的剪辑或转场方式","continuity":"从上一镜头继承的人物位置、动作、道具和情绪状态","referenceIndexes":[1]}],"changeSummary":"本轮修改摘要"}。这是可直接进入制作的分镜，不是剧情摘要。按时长动态生成：30 秒约 8–12 镜，60 秒约 12–18 镜，90 秒约 18–20 镜；每镜必须为 3–8 秒，禁止用大量长镜头压缩剧情。场景建立、关键动作、线索特写、人物反应和转场不能省略。生成完成前逐镜检查 continuity，确保上一镜头的结束状态与下一镜头开场在人物位置、道具、动作、视线和情绪上连续；发现跳跃时主动补充过渡镜头。每个 scene、imagePrompt、videoPrompt 可使用 80–160 个中文字符，对白自然且信息明确。referenceIndexes 是用户所选视觉素材从 1 开始的编号。首次生成要有完整起承转合；修改时保留未被要求改变的内容并落实本轮意见。`
  ...
  try{
    const proxyUrl=String(process.env.PROMPT_AGENT_HTTPS_PROXY||process.env.OPENAI_IMAGE_HTTPS_PROXY||''),startedAt=Date.now(),options={method:'POST',headers:{authorization:`Bearer ${apiKey}`,'content-type':'application/json'},body:JSON.stringify({model,stream:true,stream_options:{include_usage:true},reasoning_effort:'low',temperature:.42,max_tokens:6500,response_format:{type:'json_object'},messages:[{role:'system',content:system},{role:'user',content}]}),signal:AbortSignal.timeout(180000)},response=proxyUrl?await undiciFetch(`${baseUrl}/v1/chat/completions`,{...options,dispatcher:new ProxyAgent(proxyUrl)}):await fetch(`${baseUrl}/v1/chat/completions`,options)
    if(!response.ok){const payload=await response.json() as {error?:{message?:string}};return reply.code(response.status).send({error:payload.error?.message||`漫剧策划接口返回 ${response.status}`})}
    if(!response.body)throw new Error('漫剧策划接口未返回响应流')
    ...
    const reader=response.body.getReader(),decoder=new TextDecoder();let buffer='',raw='',lastProgress=0
    while(true){const {done,value}=await reader.read();if(done)break;buffer+=decoder.decode(value,{stream:true});const lines=buffer.split(/\r?\n/);buffer=lines.pop()||'';for(const line of lines){const data=line.startsWith('data:')?line.slice(5).trim():'';if(!data||data==='[DONE]')continue;try{const packet=JSON.parse(data) as {choices?:Array<{delta?:{content?:string}}>},delta=String(packet.choices?.[0]?.delta?.content||'');if(!delta)continue;raw+=delta;const progress=Math.min(92,8+Math.floor(raw.length/28));if(progress>=lastProgress+3){lastProgress=progress;const phase=raw.includes('"shots"')?'正在拆分关键分镜…':raw.includes('"outline"')?'正在组织剧情大纲…':raw.includes('"characters"')?'正在塑造角色…':'正在构思故事核心…';emit({type:'progress',progress,phase})}}catch{/* ignore upstream keepalive lines */}}}
    if(streamHeartbeat){clearInterval(streamHeartbeat);streamHeartbeat=null}raw=raw.replace(/^```(?:json)?\s*/i,'').replace(/\s*```$/,'').trim();const plan=JSON.parse(raw) as Record<string,unknown>,rawShots=Array.isArray(plan.shots)?plan.shots:[]
    request.log.info({model,elapsedMs:Date.now()-startedAt,responseLength:raw.length},'comic agent response received')
    const shots=rawShots.slice(0,20).map((value,index)=>{const shot=value&&typeof value==='object'?value as Record<string,unknown>:{};return {number:index+1,title:String(shot.title||`镜头 ${index+1}`).slice(0,50),duration:Math.max(3,Math.min(8,Number(shot.duration)||5)),scene:String(shot.scene||'').slice(0,800),dialogue:String(shot.dialogue||'').slice(0,500),imagePrompt:compactImagePrompt(String(shot.imagePrompt||shot.scene||'')),videoPrompt:String(shot.videoPrompt||'').slice(0,800),transition:String(shot.transition||'').slice(0,300),continuity:String(shot.continuity||'').slice(0,500),referenceIndexes:Array.isArray(shot.referenceIndexes)?[...new Set(shot.referenceIndexes.map(Number).filter(number=>Number.isInteger(number)&&number>=1&&number<=visualInputs.length))]:[]}}).filter(shot=>shot.imagePrompt&&shot.videoPrompt)
    if(!shots.length)throw new SyntaxError('missing shots')
    const characters=(Array.isArray(plan.characters)?plan.characters:[]).slice(0,12).map(value=>{const character=value&&typeof value==='object'?value as Record<string,unknown>:{};return {name:String(character.name||'未命名角色').slice(0,50),description:String(character.description||'').slice(0,800)}}),outline=(Array.isArray(plan.outline)?plan.outline:[]).slice(0,8).map((value,index)=>{const item=value&&typeof value==='object'?value as Record<string,unknown>:{};return {act:String(item.act||`第 ${index+1} 幕`).slice(0,50),content:String(item.content||'').slice(0,1200)}})
    const result={title:String(plan.title||'未命名漫剧').slice(0,100),logline:String(plan.logline||'').slice(0,600),tone:String(plan.tone||'').slice(0,300),duration:duration==='由对话内容推断'?String(plan.duration||`${shots.reduce((sum,shot)=>sum+shot.duration,0)} 秒`).slice(0,30):duration,aspectRatio:aspectRatio==='由对话内容推断'?(['9:16','16:9','1:1'].includes(String(plan.aspectRatio))?String(plan.aspectRatio):'9:16'):aspectRatio,characters,outline,shots,changeSummary:String(plan.changeSummary||'').slice(0,300),model};emit({type:'result',data:result});reply.raw.end();return
...
app.get('/admin/feedback', async (request, reply) => { const user=requireUser(request,reply);if(!user)return;if(!Boolean(user.isAdmin))return reply.code(403).send({error:'仅管理员可查看反馈'});return getAll('SELECT f.id,f.type,f.title,f.content,f.contact,f.project_id AS projectId,f.page_url AS pageUrl,f.status,f.created_at AS createdAt,u.name AS userName,u.username FROM feedback f JOIN users u ON u.id=f.user_id ORDER BY f.created_at DESC LIMIT 500',[]) })
