  ...
  try{
    ...
    for(let attempt=0;attempt<candidateModels.length;attempt++){
      ...
      try{
        ...
        if(candidate.ok&&candidate.body){responseBody=candidate.body as ReadableStream<Uint8Array>;upstreamController=controller;break}
        const failure=await candidate.text();lastUpstreamError=`${candidate.status} ${failure.slice(0,300)}`;request.log.warn({attempt:attempt+1,model:usedModel,status:candidate.status},'comic agent upstream unavailable')
