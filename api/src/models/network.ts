...
export async function discoverModels(connection: ProviderConnection) {
  try {
    const response = await modelFetch(`${apiRoot(connection.baseUrl)}/v1/models`, { headers: { authorization: `Bearer ${connection.apiKey}` }, signal: AbortSignal.timeout(15_000) }, connection.proxyUrl, 2 * 1024 * 1024)
