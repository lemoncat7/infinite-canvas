...
任务的 `input_urls` 也会同步为“参考图片 → 本次生成结果”的画布连线，保留输入顺序。优先复用已有图片节点；提交时会为同项目中仅在资产库的参考图片补建卡片。连线指向本次结果，不修改可重复使用的生成卡片当前配置。旧任务可调用 `viora_generation_sync` 补线；需要补建参考图卡片时传 `createMissing:true`。普通轮询不会恢复已经同步后被用户删除的参考连线。无法匹配的输入返回 `referencesSync: pending` 和未匹配序号，不抓取外部 URL，也不重新生成。
